"""
Light SQLite store for NightLife catalog — filterable posts; full post blobs as JSON.
...
Or: gallery_app auto-imports if DB missing or older than JSON.
"""
...
DB_PATH = ROOT / "back-end" / "data" / "nightlife_catalog.db"
...
CREATE TABLE IF NOT EXISTS posts (
  ...
  event_date TEXT,
  event_date_end TEXT,
  ticket_price_mkd REAL,
  listing_type TEXT,
  performer_roles TEXT,
...
def _migrate_posts_event_columns(conn: sqlite3.Connection) -> None:
    cols = {r[1] for r in conn.execute("PRAGMA table_info(posts)").fetchall()}
    if "event_date" not in cols:
        conn.execute("ALTER TABLE posts ADD COLUMN event_date TEXT")
    if "event_date_end" not in cols:
        conn.execute("ALTER TABLE posts ADD COLUMN event_date_end TEXT")
    if "performer_roles" not in cols:
        conn.execute("ALTER TABLE posts ADD COLUMN performer_roles TEXT")
    cols_after = {r[1] for r in conn.execute("PRAGMA table_info(posts)").fetchall()}
    if "event_date" in cols_after:
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_posts_event_date ON posts(event_date)"
        )


def _migrate_suggest_blocks(conn: sqlite3.Connection) -> None:
    conn.execute(
        """
...
def _migrate_suggest_jobs(conn: sqlite3.Connection) -> None:
    conn.execute(
        """
...
def _migrate_suggest_submit_log(conn: sqlite3.Connection) -> None:
    conn.execute(
        """
...
def init_schema(conn: sqlite3.Connection) -> None:
    ...
    _migrate_posts_event_columns(conn)
    _migrate_suggest_blocks(conn)
    _migrate_suggest_jobs(conn)
    _migrate_suggest_submit_log(conn)
...
def sync_from_json(catalog: dict | None = None) -> tuple[int, int]:
    ...
    with closing(get_connection()) as conn:
        ...
        for username in sorted(by_u.keys()):
            ...
            conn.execute(
                "INSERT INTO accounts (username, profile_json) VALUES (?, ?)",
            ...
            for post in block.get("posts") or []:
                if not isinstance(post, dict):
                    continue
                pj = json.dumps(post, ensure_ascii=False)
                ts = (post.get("timestamp") or "").strip() or None
                pdate = _posted_date_str(post)
                ev_start, ev_end = _event_date_range_from_post(post)
                ticket, listing = _ticket_and_listing(post)
                proles = _performer_roles_json(post)
                vis = 1 if post_visible_on_site(post) else 0
                conn.execute(
...
def ensure_database() -> None:
    """Create DB and import from JSON if DB missing, empty, or older than the JSON catalog."""
    if not CATALOG_JSON_PATH.is_file():
        return
    need = True
    try:
        j_mtime = CATALOG_JSON_PATH.stat().st_mtime
    except OSError:
        return
    if DB_PATH.is_file():
        try:
            d_mtime = DB_PATH.stat().st_mtime
            with closing(get_connection()) as conn:
                n = conn.execute("SELECT COUNT(*) AS c FROM posts").fetchone()["c"]
            need = n == 0 or j_mtime > d_mtime
...
def account_exists(username: str) -> bool:
    ...
    with closing(get_connection()) as conn:
        r = conn.execute(
            "SELECT 1 FROM accounts WHERE username = ? LIMIT 1", (username,)
        ).fetchone()
...
def suggest_submit_is_blocked(ip_hash: str) -> bool:
    ...
    with closing(get_connection()) as conn:
        r = conn.execute(
            "SELECT created_at FROM suggest_submit_blocks WHERE ip_hash = ? LIMIT 1",
            (ip_hash,),
        ).fetchone()
        if not r:
            return False
        created = _parse_block_created_at(r["created_at"])
        if created is None:
            return True
...
def suggest_log_submission(ip_hash: str) -> None:
    ...
    with closing(get_connection()) as conn:
        conn.execute(
            "INSERT INTO suggest_submit_log (ip_hash, created_at) VALUES (?, ?)",
            (ip_hash, now),
        )
        conn.commit()
...
def suggest_rate_limited(ip_hash: str) -> bool:
    ...
    with closing(get_connection()) as conn:
        row = conn.execute(
            "SELECT COUNT(*) AS c FROM suggest_submit_log WHERE ip_hash = ? AND created_at >= ?",
            (ip_hash, since),
        ).fetchone()
...
def suggest_job_claim_next() -> dict | None:
    ...
    with closing(get_connection()) as conn:
        ...
        job_id = int(r["id"])
        conn.execute(
            "UPDATE suggest_jobs SET status = 'running', updated_at = ? WHERE id = ?",
            (now, job_id),
        )
        conn.execute("COMMIT")
...
def suggest_job_finish(
    ...
    with closing(get_connection()) as conn:
        conn.execute(
            """
            UPDATE suggest_jobs
            SET status = 'done', ok = ?, message = ?, canonical_username = ?, updated_at = ?
            WHERE id = ?
            """,
            (1 if ok else 0, (message or "")[:800], canonical_username, now, job_id),
        )
        conn.commit()
...
def _weekday_clause(weekday: str | None) -> tuple[str, list[Any]]:
    ...
    if w not in _WEEKDAY_SQL:
        return "", []
    return (
        " AND p.event_date IS NOT NULL AND trim(p.event_date) != '' "
        "AND strftime('%w', date(substr(trim(p.event_date), 1, 10))) = ?",
        [_WEEKDAY_SQL[w]],
    )
...
def _date_clause(
    ...
    if date_from is not None:
        cond.append(
            "(event_date IS NOT NULL AND COALESCE(event_date_end, event_date) >= ?)"
        )
        args.append(date_from.isoformat())
    if date_to is not None:
        cond.append("(event_date IS NOT NULL AND event_date <= ?)")
...
def user_blocks_from_db(
    ...
    sql = (
        ...
        "WHERE p.visible = 1" + ds + hs + " ORDER BY p.username COLLATE NOCASE, p.posted_at DESC"
    )
    with closing(get_connection()) as conn:
...
def _username_clause(username: str | None) -> tuple[str, list[Any]]:
    if not username or not str(username).strip():
        return "", []
    return " AND p.username = ?", [username.strip()]
...
def _search_clause(search: str | None) -> tuple[str, list[Any]]:
    ...
    return (
        ...
        "OR LOWER(IFNULL(json_extract(a.profile_json, '$.full_name'), '')) LIKE ? "
        "OR LOWER(p.post_json) LIKE ?)",
        [pat, pat, pat],
    )


def _role_clause(performer_role: str | None) -> tuple[str, list[Any]]:
    """Match a role inside JSON array stored in performer_roles (e.g. [\"dj\",\"singer\"])."""
    if not performer_role or performer_role not in _VALID_ROLES:
        return "", []
    return (
        " AND p.performer_roles IS NOT NULL AND p.performer_roles LIKE ?"
    ), [f'%"{performer_role}"%']
...
def _where_visible_and_filters(
    ...
    hs, hargs = _hidden_exclude_posts_sql("p")
    return (
        " WHERE p.visible = 1" + ds + ps + ws + us + rs + ss + hs,
...
_ORDER_FLAT = (
    " ORDER BY "
    "CASE WHEN p.event_date IS NULL OR trim(p.event_date) = '' THEN 1 ELSE 0 END ASC, "
    "p.event_date ASC, "
...
_ORDER_BY_POSTED = (
    " ORDER BY "
    "CASE WHEN p.posted_at IS NULL OR trim(p.posted_at) = '' THEN 1 ELSE 0 END ASC, "
    "p.posted_at DESC"
)
...
if __name__ == "__main__":
    ...
    print(f"Synced {CATALOG_JSON_PATH} -> {DB_PATH}")
